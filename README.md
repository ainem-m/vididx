...
`vidx` は、ローカルの mp4 動画を RAG 向けの JSONL チャンク群と人間可読な Markdown に変換する Rust 製 CLI です。

ASR・シーン変化検出・粗分割・意味分割・正規化・出力生成を cargo workspace として分離し、将来的な外部 API / ローカルツール差し替えを前提に設計しています。

## 何ができるか

- mp4 を入力にして retrieval-ready な JSONL を生成
...
- 部分実装: pipeline runner、semantic chunking の LLM 統合、注釈生成
- 未完了: OCR / VLM の本統合、CLI の pre-flight check、`--from/--to` の段階実行
