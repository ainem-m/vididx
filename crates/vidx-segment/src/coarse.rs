use vidx_core::{CoarseSegment, SceneChange, SilenceInterval, TranscriptTimeline, VidxError};
...
pub fn coarse_segment(
    ...
    max_duration: f64,
    snap_window: f64,
) -> Result<Vec<CoarseSegment>, VidxError> {
    if duration_sec <= 0.0 {
        return Err(VidxError::Segment("Duration must be positive".to_string()));
    }
