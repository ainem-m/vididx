...
use tera::Tera;
use vidx_core::{CoarseSegment, SemanticChunk, VidxError};
...
fn render_semantic_prompt(
    ...
    let template_content = if std::path::Path::new(&template_file).exists() {
        std::fs::read_to_string(&template_file)
            .map_err(|e| VidxError::Segment(format!("Failed to read template: {}", e)))?
    } else {
...
fn parse_and_validate_chunks(
    ...
    let chunk_response: ChunkResponse = serde_json::from_value(response.clone())
        .map_err(|e| VidxError::Segment(format!("Failed to parse chunks: {}", e)))?;

    let mut chunks = Vec::new();

    // Validate chunks
    if chunk_response.chunks.is_empty() {
        return Err(VidxError::Segment("No chunks in response".to_string()));
    }
