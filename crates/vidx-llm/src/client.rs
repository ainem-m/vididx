...
impl AnthropicClient {
    ...
    async fn call_api_internal(
        ...
        .await
        .map_err(|_| VidxError::Llm("Request timeout (30s)".to_string()))?
        .map_err(|e| VidxError::Llm(format!("HTTP request failed: {}", e)))?;
        ...
        let body = response
            ...
            .await
            .map_err(|e| VidxError::Llm(format!("Failed to parse response: {}", e)))?;
        ...
        let text = body
            ...
            .and_then(|t| t.as_str())
            .ok_or_else(|| VidxError::Llm("No text in response".to_string()))?;

        // Parse JSON from response text
        serde_json::from_str(text)
            .map_err(|e| VidxError::Llm(format!("Failed to parse JSON from response: {}", e)))
    }
}
