...
use tera::Tera;
use vididx_core::{AnnotatedChunk, NormalizedChunk, VididxError};
...
) -> Result<AnnotatedChunk, VididxError> {
...
fn render_annotation_prompt(chunk: &NormalizedChunk) -> Result<String, VididxError> {
...
) -> Result<AnnotationResponse, VididxError> {
    serde_json::from_value(response.clone())
        .map_err(|e| VididxError::Segment(format!("Failed to parse annotation: {}", e)))
}
