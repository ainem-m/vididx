...
use vidx_core::{MediaProbe, VidxError};
...
pub async fn probe(ffprobe_path: &str, mp4_path: &Path) -> Result<MediaProbe, VidxError> {
    ...
    let output = Command::new(ffprobe_path)
        ...
        .arg(mp4_path)
        .output()
        .map_err(|e| VidxError::Media(format!("ffprobe execution failed: {}", e)))?;
    ...
    let stdout = String::from_utf8(output.stdout)
        .map_err(|e| VidxError::Media(format!("ffprobe output not UTF-8: {}", e)))?;

    parse_ffprobe_output(&stdout)
}

/// Parse ffprobe JSON output into MediaProbe.
fn parse_ffprobe_output(json_str: &str) -> Result<MediaProbe, VidxError> {
    let probe: Value = serde_json::from_str(json_str)
        .map_err(|e| VidxError::Media(format!("Invalid ffprobe JSON: {}", e)))?;
