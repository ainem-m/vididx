...
/// Utterance-mode segmentation config.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct UtteranceSegmentConfig {
    /// Merge adjacent utterances when the silence gap between them is at most this many seconds.
    pub merge_gap_sec: f64,
    /// Merge an utterance with the next one when its duration is shorter than this.
    pub min_duration_sec: f64,
}

impl Default for UtteranceSegmentConfig {
    fn default() -> Self {
        Self {
            // Negative value disables gap-based merging; whisper already handles
            // sentence boundaries, so 0-gap between segments does not mean they
            // should be merged.  Set to e.g. 0.5 to merge across short silences.
            merge_gap_sec: -1.0,
...
pub struct SegmentConfig {
    ...
    pub utterance: UtteranceSegmentConfig,
...
impl Config {
    ...
    /// Tries: explicit path → ./vididx.toml → ~/.config/vididx/config.toml → default
    pub fn load(toml_path: Option<&Path>) -> Result<Self, VididxError> {
        ...
        if let Some(path) = path_to_try.filter(|p| p.exists()) {
            ...
            let toml_value: toml::Table = toml::from_str(&contents)
                .map_err(|e| VididxError::Config(format!("Invalid TOML: {}", e)))?;
    ...
    /// Find default config path: ./vididx.toml → ~/.config/vididx/config.toml
    fn find_default_config_path() -> Option<PathBuf> {
        let local = PathBuf::from("./vididx.toml");
        if local.exists() {
            return Some(local);
        }

        if let Some(home) = dirs::home_dir() {
            let home_config = home.join(".config/vididx/config.toml");
...
"#;
    ...
    #[test]
    fn test_segment_mode_parses_utterance_from_toml() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("vididx.toml");
        let content = r#"
[segment]
...
"#;
    ...
    #[test]
    fn test_segment_mode_parses_chapter_from_toml() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("vididx.toml");
        fs::write(&config_path, "[segment]\nmode = \"chapter\"\n").unwrap();
        let cfg = Config::load(Some(&config_path)).unwrap();
        assert_eq!(cfg.segment.mode, SegmentMode::Chapter);
    }

    #[test]
    fn test_utterance_config_defaults() {
        let cfg = Config::default();
        assert_eq!(cfg.segment.utterance.merge_gap_sec, -1.0);
        assert_eq!(cfg.segment.utterance.min_duration_sec, 3.0);
    }
