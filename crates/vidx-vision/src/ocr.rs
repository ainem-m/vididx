...
impl OcrAdapter for TesseractAdapter {
    async fn extract_text(
        ...
        let output = Command::new(&self.binary_path)
            ...
            .arg(&lang_arg)
            .output()
            .map_err(|e| VidxError::Vision(format!("tesseract execution failed: {}", e)))?;

        if !output.status.success() {
            return Err(VidxError::Vision(format!(
                "tesseract failed: {}",
        ...
        let text = String::from_utf8(output.stdout)
            .map_err(|e| VidxError::Vision(format!("tesseract output not UTF-8: {}", e)))?;
