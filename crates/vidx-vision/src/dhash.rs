...
pub fn dhash_from_path(path: &Path) -> Result<u64, VidxError> {
    let img = ImageReader::open(path)
        .map_err(|e| VidxError::Vision(format!("Failed to open image: {}", e)))?
        .decode()
        .map_err(|e| VidxError::Vision(format!("Failed to decode image: {}", e)))?;

    Ok(dhash_from_image(&img))
}

/// Compute dHash from image bytes.
pub fn dhash_from_bytes(data: &[u8]) -> Result<u64, VidxError> {
