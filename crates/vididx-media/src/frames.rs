...
pub async fn extract_frames(
    ...
    out_dir: &Path,
    quality: u8,
) -> Result<Vec<PathBuf>, VididxError> {
    ...
    for handle in handles {
        match handle.await {
            ...
            Err(e) => return Err(VididxError::Media(format!("Task join error: {}", e))),
        }
    }
...
fn extract_single_frame(
    ...
    let output = Command::new(ffmpeg_path)
        ...
        .arg(&output_path)
        .output()
        .map_err(|e| VididxError::Media(format!("ffmpeg frame extraction failed: {}", e)))?;
