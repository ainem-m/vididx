...
use vididx_core::{SceneChange, VididxError};
...
fn parse_timecode(tc: &str) -> Result<f64, VididxError> {
    let parts: Vec<&str> = tc.split(':').collect();
    if parts.len() != 3 {
        return Err(VididxError::Media(format!("Invalid timecode: {}", tc)));
    }

    let hours: f64 = parts[0]
        .parse()
        .map_err(|_| VididxError::Media(format!("Invalid hours in timecode: {}", tc)))?;
    let minutes: f64 = parts[1]
        .parse()
        .map_err(|_| VididxError::Media(format!("Invalid minutes in timecode: {}", tc)))?;
    let seconds: f64 = parts[2]
        .parse()
        .map_err(|_| VididxError::Media(format!("Invalid seconds in timecode: {}", tc)))?;
