...
pub async fn write_chunks_jsonl(
    ...
    .map_err(VidxError::Io)?;

    let file = File::create(output_path).map_err(VidxError::Io)?;
    let mut writer = BufWriter::new(file);

    for chunk in chunks {
        let json = serde_json::to_string(chunk).map_err(VidxError::Serde)?;
        writeln!(writer, "{}", json).map_err(VidxError::Io)?;
    }

    writer.flush().map_err(VidxError::Io)?;

    Ok(())
}

/// Write an index JSON file.
pub async fn write_index(index: &OutputIndex, output_path: &Path) -> Result<(), VidxError> {
    create_dir_all(output_path.parent().ok_or_else(|| {
        VidxError::Io(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            "Invalid output path",
    ...
    let json = serde_json::to_string_pretty(index).map_err(VidxError::Serde)?;

    std::fs::write(output_path, json).map_err(VidxError::Io)?;
...
pub async fn write_markdown(
    ...
    for (idx, chunk) in chunks.iter().enumerate() {
        ...
        writeln!(writer, "**Summary:** {}\n", chunk.summary).map_err(VidxError::Io)?;

        writeln!(writer, "**Keywords:** {}\n", chunk.keywords.join(", ")).map_err(VidxError::Io)?;
